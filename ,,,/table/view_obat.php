	<table>
		...
		<th><input type="search" placeholder="data yang ingin di cari" id="cari_obat_value" required></th>
		 ...
		 $limit = 'LIMIT '.($page_saatini-1)*$page_rows.','.$page_rows;
			if(@$_POST['cari_obat']!=null || @$_POST['cari_obat']!=""){
				$cari = @$_POST['cari_obat'];
				 $sql="SELECT kode_item,nama_item,kategori,stok,satuan FROM tbl_obat WHERE nama_item='$cari' $limit";
			}else{
