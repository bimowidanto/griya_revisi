	<table>
		...
		<th class="right"><a class="btn-floating waves-effect" id="cari_brgmasuk"><i class="mdi-action-search"></i></a></th>
	...
	<table class="striped">
		...
		<tbody>
		 ...
		 $limit = 'LIMIT '.($page_saatini-1)*$page_rows.','.$page_rows;
			if(@$_POST['cari_brg']!=null || @$_POST['cari_brg']!=""){
				$cari = @$_POST['cari_brg'];
				$sql="SELECT tbl_barang_masuk.id_barang,tbl_barang_masuk.kode_item,tbl_obat.nama_item,tbl_barang_masuk.stok,tbl_barang_masuk.expired_date,tbl_barang_masuk.harga_beli,tbl_barang_masuk.harga_jual,tbl_barang_masuk.id_supplier,tbl_supplier.nama_supplier,tbl_barang_masuk.tgl_masuk FROM tbl_supplier,tbl_barang_masuk,tbl_obat WHERE tbl_obat.nama_item LIKE'%$cari%' AND tbl_barang_masuk.kode_item=tbl_obat.kode_item AND tbl_barang_masuk.id_supplier=tbl_supplier.id_supplier ORDER BY tbl_barang_masuk.id_barang ASC";
